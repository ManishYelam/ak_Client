...
const Payment = ({ amount, onPaymentSuccess, onBack, userData = {} }) => {
  ...
  const handleRazorpayPayment = useCallback(async () => {
    ...
    try {
      ...
      const options = {
        ...
        modal: {
          ondismiss: () => {
            showInfoToast(
              `Payment of ${formatAmount(amount)} cancelled. You can retry or use UPI payment.`
            )
          },
        },
        retry: {
      ...
      rzp.on('payment.failed', function (response) {
        ...
        showErrorToast(
          `Payment of ${formatAmount(amount)} failed: ${response.error.description}. Please try UPI payment.`
        )
      })
    ...
    loadRazorpayScript,
    onPaymentSuccess,
    convertBackendResponseToRupees,
  ])
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md">
      ...
      <div className="text-center mb-8">
        ...
        <div className="text-3xl font-bold text-green-600">{formatAmount(amount)}</div>
...
const PaymentMethodSelector = React.memo(
  ({ paymentMethod, onMethodChange, amount, formatAmount }) => (
...
const PaymentMethodButton = React.memo(
  ({ method, currentMethod, icon, title, description, color, onMethodChange }) => {
    ...
    return (
      <button
        ...
        className={`w-full p-4 border-2 rounded-lg text-left transition-all ${
          isSelected ? colorClasses[color].selected : 'border-gray-300 bg-white hover:bg-gray-50'
        }`}
      >
        <div className="flex items-center gap-3">
          <div
            className={`w-10 h-10 rounded-full flex items-center justify-center ${
              isSelected ? colorClasses[color].icon : 'bg-gray-100'
            }`}
          >
...
const RazorpayPaymentSection = React.memo(
  ({ loading, onPayment, onMethodChange, amount, formatAmount }) => (
    <div className="bg-green-50 border border-green-200 rounded-lg p-6">
      <h4 className="font-semibold text-green-800 mb-4 text-lg">Pay with Razorpay</h4>
      <p className="text-green-700 text-sm mb-6">
        You'll be redirected to Razorpay's secure payment gateway to complete your payment of{' '}
        {formatAmount(amount)} safely.
      </p>
      ...
      <div className="space-y-3">
        <label className="block">
          ...
          <div className="w-full px-4 py-3 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer bg-white hover:bg-gray-50 transition-colors text-center">
            {screenshot
              ? 'Change Screenshot'
              : `Upload Payment Confirmation Screenshot for ${formatAmount(amount)}`}
          </div>
        </label>
  ...
  <>
    <div className="flex items-center justify-between p-4 bg-white rounded-lg border border-gray-200">
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="text-sm font-medium text-gray-800">{screenshot.name}</p>
          <p className="text-xs text-gray-500">{(screenshot.size / 1024 / 1024).toFixed(2)} MB</p>
        </div>
      </div>
      <button onClick={onRemoveScreenshot} className="text-red-500 hover:text-red-700 p-2">
        ×
      </button>
    </div>
