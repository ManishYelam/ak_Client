...
const CaseFormDetails = ({
  ...
  const getFieldBorderClass = useCallback(
    fieldName => {
  ...
  const getInputDisabled = useCallback(
    fieldName => {
    ...
    try {
      ...
      showSuccessToast(
        mode === 'edit'
          ...
          : 'Deposit details filled successfully!'
      )
      onNext()
    } catch (error) {
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="flex flex-col gap-2">
          <label className="font-semibold">{mandatoryLabel('Saving Account Starting Date')}</label>
          <input
            type="date"
            ...
            className={`p-1 border rounded text-[10px] ${getFieldBorderClass('saving_account_start_date')} ${
              getInputDisabled('saving_account_start_date')
          ...
          <label className="font-semibold">{mandatoryLabel('Deposit Type')}</label>
          <select
            name="deposit_type"
            ...
            className={`p-1 border rounded text-[10px] ${getFieldBorderClass('deposit_type')} ${
              getInputDisabled('deposit_type') ? 'bg-gray-50 text-gray-600 cursor-not-allowed' : ''
            }`}
            required
          >
            ...
            <option value="Investment Scheme (Exhibit A)">Investment Scheme (Exhibit A)</option>
          </select>
          {touchedFields.deposit_type && fieldErrors.deposit_type && (
            <p className="text-red-500 text-[9px]">{fieldErrors.deposit_type}</p>
          )}

          <label className="font-semibold">{mandatoryLabel('Deposit Duration (Years)')}</label>
          <input
            type="number"
            ...
            className={`p-1 border rounded text-[10px] ${getFieldBorderClass('deposit_duration_years')} ${
              getInputDisabled('deposit_duration_years')
                ? 'bg-gray-50 text-gray-600 cursor-not-allowed'
                : ''
            }`}
            min="0"
        ...
        <div className="flex flex-col gap-2">
          <label className="font-semibold">{mandatoryLabel('Fixed Deposit Total Amount')}</label>
          <input
            type="number"
            ...
            className={`p-1 border rounded text-[10px] ${getFieldBorderClass('fixed_deposit_total_amount')} ${
              getInputDisabled('fixed_deposit_total_amount')
                ? 'bg-gray-50 text-gray-600 cursor-not-allowed'
                : ''
            }`}
            min="0"
          ...
          <label className="font-semibold">{mandatoryLabel('Interest Rate (FD %)')}</label>
          <input
            type="number"
            ...
            className={`p-1 border rounded text-[10px] ${getFieldBorderClass('interest_rate_fd')} ${
              getInputDisabled('interest_rate_fd')
                ? 'bg-gray-50 text-gray-600 cursor-not-allowed'
                : ''
            }`}
            min="0"
          ...
          <label className="font-semibold">{mandatoryLabel('Savings Account Total Amount')}</label>
          <input
            type="number"
            ...
            className={`p-1 border rounded text-[10px] ${getFieldBorderClass('saving_account_total_amount')} ${
              getInputDisabled('saving_account_total_amount')
                ? 'bg-gray-50 text-gray-600 cursor-not-allowed'
                : ''
            }`}
            min="0"
          ...
          <label className="font-semibold">{mandatoryLabel('Interest Rate (Savings %)')}</label>
          <input
            type="number"
            ...
            className={`p-1 border rounded text-[10px] ${getFieldBorderClass('interest_rate_saving')} ${
              getInputDisabled('interest_rate_saving')
                ? 'bg-gray-50 text-gray-600 cursor-not-allowed'
                : ''
            }`}
            min="0"
        ...
        <div className="flex flex-col gap-2">
          ...
          <input
            ...
            className={`p-1 border rounded text-[10px] ${getFieldBorderClass('recurring_deposit_total_amount')} ${
              getInputDisabled('recurring_deposit_total_amount')
                ? 'bg-gray-50 text-gray-600 cursor-not-allowed'
                : ''
            }`}
            min="0"
            ...
            required
          />
          {touchedFields.recurring_deposit_total_amount &&
            fieldErrors.recurring_deposit_total_amount && (
              <p className="text-red-500 text-[9px]">
                {fieldErrors.recurring_deposit_total_amount}
              </p>
            )}

          <label className="font-semibold">{mandatoryLabel('Interest Rate (RD %)')}</label>
          <input
            type="number"
            ...
            className={`p-1 border rounded text-[10px] ${getFieldBorderClass('interest_rate_recurring')} ${
              getInputDisabled('interest_rate_recurring')
                ? 'bg-gray-50 text-gray-600 cursor-not-allowed'
                : ''
            }`}
            min="0"
          ...
          <input
            ...
            className={`p-1 border rounded text-[10px] ${getFieldBorderClass('dnyanrudha_investment_total_amount')} ${
              getInputDisabled('dnyanrudha_investment_total_amount')
                ? 'bg-gray-50 text-gray-600 cursor-not-allowed'
                : ''
            }`}
            min="0"
            ...
            required
          />
          {touchedFields.dnyanrudha_investment_total_amount &&
            fieldErrors.dnyanrudha_investment_total_amount && (
              <p className="text-red-500 text-[9px]">
                {fieldErrors.dnyanrudha_investment_total_amount}
              </p>
            )}

          <label className="font-semibold">{mandatoryLabel('Dynadhara Rate (%)')}</label>
          <input
            type="number"
            name="dynadhara_rate"
            ...
            className={`p-1 border rounded text-[10px] ${getFieldBorderClass('dynadhara_rate')} ${
              getInputDisabled('dynadhara_rate')
                ? 'bg-gray-50 text-gray-600 cursor-not-allowed'
                : ''
            }`}
            min="0"
      ...
      <div className="mt-4 p-2 bg-gray-100 rounded text-[8px] text-gray-600">
        <strong>Debug Info:</strong> Mode: {mode} | Loading: {isLoading ? 'Yes' : 'No'} | Form
        Valid: {isFormValid ? 'Yes' : 'No'} | Has Data: {formData.deposit_type ? 'Yes' : 'No'}
      </div>
