...
const CaseDocumentUploader = ({
  ...
  const handleFilesChange = async (docKey, files) => {
    ...
    try {
      ...
      const updatedDocs = [...documents.filter(d => d.exhibit !== docKey), ...processedFiles]
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-white rounded-lg shadow-md">
      ...
      <div className="mb-4 p-2 bg-blue-50 border border-blue-200 rounded text-[9px] text-blue-700">
        <strong>Debug:</strong> Mode: {mode} | Loading: {isLoading ? 'Yes' : 'No'} | Exhibit:{' '}
        {exhibit} | Uploaded: {status.uploadedCount}/{status.totalCount} | Last Step:{' '}
        {isLastStep ? 'Yes' : 'No'} | Submitting: {isSubmitting ? 'Yes' : 'No'} | Complete:{' '}
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="text-sm text-gray-600">
            {status.uploadedCount} of {status.totalCount} documents{' '}
            {mode === 'edit' ? 'available' : 'uploaded'}
            {mode === 'edit' && ' (optional)'}
          </div>
        </div>
        ...
        <div className="w-full bg-gray-200 rounded-full h-2 mb-2">
          <div
            className={`h-2 rounded-full transition-all duration-300 ${
              mode === 'edit'
                ? 'bg-blue-400' // Different color for edit mode (optional)
                : status.isComplete
                  ? 'bg-green-600'
                  : 'bg-blue-600'
            }`}
