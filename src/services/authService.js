...
export const verifyEmailService = (userId, otp) =>
  axiosInstance.post(`/users/verify?userId=${userId}&otp=${otp}`)

export const resendVerification = userId =>
  axiosInstance.post(`/users/resend-verification?userId=${userId}`)
...
export const otpChangePasswordService = (email, data) =>
  axiosInstance.post(`/reset-password/${email}`, data)
